...
class NeuralNetwork:
    def __init__(self, input_shape: tuple[int, ...], layers: list[Layer], optimizer: Optimizer, loss: Loss, metrics: any, verbose: bool=False) -> None:
        """
...
class ConvolutionalNeuralNetwork(NeuralNetwork):
    ...
    # TODO: Verificar funcionamiento con distintos modelos y imagenes.
    def get_gradcam(self, data: np.ndarray, alpha: float=1.0) -> np.ndarray:
        ...
        :return gradcam: matriz de gradiente CAM
        """
        x = data
        ...
        grad_pooled = grad_dense_1.reshape(32, 1, 13, 13)
        feature_maps = feature_maps.transpose(1, 0, 2, 3)

        maxpool = self.layers[1].copy()
        ...
        jet_heatmap = jet_heatmap.resize((28, 28), resample=Image.NEAREST)
        jet_heatmap = np.array(jet_heatmap, dtype=np.float32) / 255.0

        img = data[0].reshape(28, 28, 1) / 255
